...
from app.core.memory_store import add_memory, retrieve_memories
...
class DramaProductionState(TypedDict, total=False):
    ...
    # #10 集级别断点续传
    _completed_stages: List[str]           # 已完成的阶段
    _completed_episodes: List[int]         # 已完成扩写的集号
    failed_dimensions: List[str]           # #7 未通过的审核维度
    previous_review_feedback: str          # #7 上次审核反馈（供重试注入）
...
class DirectorBrainAgent:
    ...
    system_prompt = load_prompt_or("director_brain", """
【身份】
...
def _record_error_case(
    ...
    try:
        add_memory(
            ...
            stage=stage,
            agent=agent,
            content=f"[{issue_type}] {location}: {description} → 修复: {correction}",
            metadata={"rule_version": state.get("rule_version", "1.0")},
        )
    except Exception:
        pass
...
def _evolve_rule_version(state: DramaProductionState, reason: str, changes: List[str]) -> None:
    ...
    try:
        add_memory(
            memory_type="rule_update",
            stage=state.get("current_stage", ""),
            agent="director_brain",
            content=f"规则库升级 {old_version}→{new_version}: {reason}. 变更: {'; '.join(changes)}",
...
async def _log(state: DramaProductionState, role: str, agent: Optional[str], step: str, content: str, payload: Dict[str, Any] = None):
    # #4 角色化消息：注入 Agent 元数据供前端展示
    agent_meta = get_agent_meta_dict(agent) if agent else {}
    msg = {
        "role": role,
        "agent": agent,
        "agent_meta": agent_meta,  # #4 角色化元数据
        "step": step,
        "content": content,
        "payload": payload or {},
    }
...
async def _checkpoint(state: DramaProductionState) -> None:
    ...
    #10 集级别断点续传：增量保存节点产出，节点级跳过由 _should_skip_node 判断。
    注意：阶段是否“验收通过”由 Review 节点写入 _completed_stages，此处不重复标记。
    """
...
def _has_stage_output(state: DramaProductionState, stage: str) -> bool:
    """#10 检查某阶段的产出是否已存在（用于断点续传跳过）。

    判断依据：阶段对应的 key 存在且非空、非 fallback（任一存在即返回 True）。
    """
    _STAGE_OUTPUT_KEYS = {
        "planning": ["script_outline", "full_script"],
        "asset": ["character_assets", "scene_assets"],
        "production": ["storyboard_data", "video_plan"],
    }
    keys = _STAGE_OUTPUT_KEYS.get(stage, [])
    for key in keys:
        val = state.get(key)
        if isinstance(val, dict) and val and not val.get("_is_fallback"):
            return True
    return False


def _has_all_stage_outputs(state: DramaProductionState, stage: str) -> bool:
    """#10 检查某阶段的**所有**产出是否都已存在且有效（用于阶段级断点续传跳过）。

    与 _has_stage_output 的区别：要求该阶段所有 key 都存在且非 fallback。
    """
    _STAGE_OUTPUT_KEYS = {
        "planning": ["script_outline", "full_script"],
        "asset": ["character_assets", "scene_assets"],
    ...
    for key in keys:
        val = state.get(key)
        if not isinstance(val, dict) or not val or val.get("_is_fallback"):
            return False
    return True


def _should_skip_node(state: DramaProductionState, output_key: str) -> bool:
    """#10 检查节点产出是否已存在且不需要重跑（断点续传跳过条件）。

    跳过条件（全部满足才跳过）：
    1. 产出 key 存在且为非空 dict、非 fallback
    2. 无待处理的质检反馈（feedback_message 为空）—— 有反馈说明 Review 要求重做
    3. 无待处理的用户反馈（pending_user_feedback 为空）
    4. 无运行时错误（last_error 为空）—— 有错误说明上次执行失败需重试
    """
...
async def node_script_planner(state: DramaProductionState) -> DramaProductionState:
    ...
    # #10 断点续传：如果大纲已存在且无需重做，直接跳过
    if _should_skip_node(state, "script_outline"):
        existing = state.get("script_outline") or {}
        # 剧本模式/小说模式的特殊标记数据不算"已完成大纲"
        if not existing.get("is_user_script"):
            await _log(state, "agent", "script_planner", "planning",
                       "剧本大纲已存在（断点续传跳过）",
                       {"project_title": existing.get("project_title"),
                        "episodes": len(existing.get("episodes", []))})
...
async def node_screenwriter(state: DramaProductionState) -> DramaProductionState:
    ...
    # #10 断点续传：如果分场剧本已存在且无需重做，直接跳过
    if _should_skip_node(state, "full_script"):
        existing = state.get("full_script") or {}
        ep_count = len((existing.get("screenplay", {}) or {}).get("episodes", []) or [])
        await _log(state, "agent", "screenwriter", "planning",
                   f"分场剧本已存在（断点续传跳过，共 {ep_count} 集）")
        state["feedback_message"] = ""
        state["last_error"] = ""
        return state
    ...
    if _should_shard_episodes(script_outline):
        # #10 集级别断点续传：检查已有集，只处理缺失的集
        existing_full = state.get("full_script") or {}
        existing_episode_nums: set = set()
        existing_episodes: List[Dict[str, Any]] = []
        if isinstance(existing_full, dict) and not existing_full.get("_is_fallback"):
            for ep in (existing_full.get("screenplay", {}).get("episodes", []) or []):
                ep_num = ep.get("episode_num")
                if ep_num is not None:
                    existing_episode_nums.add(ep_num)
                    existing_episodes.append(ep)

        all_episode_nums = [ep.get("episode_num", i + 1) for i, ep in enumerate(script_outline.get("episodes", []) or [])]
        pending_episode_nums = [n for n in all_episode_nums if n not in existing_episode_nums]

        if not pending_episode_nums:
            await _log(state, "agent", "screenwriter", "planning",
                       f"分场剧本已完整（断点续传跳过，共 {len(existing_episode_nums)} 集）")
            state["feedback_message"] = ""
            state["last_error"] = ""
            await _checkpoint(state)
            return state

        resume_hint = f"（断点续传：已有 {len(existing_episode_nums)} 集，待扩写 {len(pending_episode_nums)} 集）" if existing_episode_nums else ""
        await _log(state, "agent", "screenwriter", "planning",
                   f"集数较多，将按每 {EPISODE_BATCH_SIZE} 集分批扩写...{resume_hint}")
        try:
            batches = [pending_episode_nums[i : i + EPISODE_BATCH_SIZE] for i in range(0, len(pending_episode_nums), EPISODE_BATCH_SIZE)]
            ...
            if failed_batches:
                ...
                # #10 保留已有集，不覆盖为 fallback（若无已有集才设 fallback 标记）
                if not existing_episode_nums:
            ...
            if existing_episodes:
                all_results = [{
                    "screenplay": {"episodes": existing_episodes, "project_title": script_outline.get("project_title", "")},
                    "style_bible": existing_full.get("style_bible", ""),
                    "color_palette": existing_full.get("color_palette", []),
                }] + all_results
            merged = _merge_screenplay_results(all_results, script_outline)
            state["full_script"] = merged
            state["feedback_message"] = ""
            state["last_error"] = ""
            total_eps = len(merged.get('screenplay', {}).get('episodes', []))
            await _log(state, "agent", "screenwriter", "planning", f"分场剧本已完成（分片扩写 {len(batches)} 批，共 {total_eps} 集）")
            await _checkpoint(state)
        except Exception as exc:
            state["last_error"] = str(exc)
...
async def node_character_designer(state: DramaProductionState) -> DramaProductionState:
    ...
    # #10 断点续传：如果角色资产已存在且无需重做，直接跳过
    if _should_skip_node(state, "character_assets"):
        existing = state.get("character_assets") or {}
        await _log(state, "agent", "character_designer", "asset",
                   f"角色资产已存在（断点续传跳过，共 {len(existing.get('characters', []))} 个角色）")
        state["feedback_message"] = ""
        state["last_error"] = ""
        return state
...
async def node_scene_prop_designer(state: DramaProductionState) -> DramaProductionState:
    ...
    # #10 断点续传：如果场景资产已存在且无需重做，直接跳过
    if _should_skip_node(state, "scene_assets"):
        existing = state.get("scene_assets") or {}
        await _log(state, "agent", "scene_prop_designer", "asset",
                   f"场景资产已存在（断点续传跳过，共 {len(existing.get('scenes', []))} 个场景）")
        state["feedback_message"] = ""
        state["last_error"] = ""
        return state
...
async def node_asset_parallel(state: DramaProductionState) -> DramaProductionState:
    ...
    # #10 断点续传：如果角色和场景资产都已存在且无需重做，直接跳过整个资产阶段
    char_ok = _should_skip_node(state, "character_assets")
    scene_ok = _should_skip_node(state, "scene_assets")
    if char_ok and scene_ok:
        char_data = state.get("character_assets") or {}
        scene_data = state.get("scene_assets") or {}
        await _log(state, "agent", "asset_parallel", "asset",
                   f"角色与场景资产均已存在（断点续传跳过，{len(char_data.get('characters', []))} 角色 + {len(scene_data.get('scenes', []))} 场景）")
        state["feedback_message"] = ""
        state["last_error"] = ""
        return state
...
async def node_storyboard_director(state: DramaProductionState) -> DramaProductionState:
    ...
    # #10 断点续传：如果分镜数据已存在且无需重做，直接跳过
    if _should_skip_node(state, "storyboard_data"):
        existing = state.get("storyboard_data") or {}
        await _log(state, "agent", "storyboard_director", "production",
                   f"分镜数据已存在（断点续传跳过，共 {len(existing.get('storyboards', []))} 个分镜）")
        state["feedback_message"] = ""
        state["last_error"] = ""
        return state
...
async def node_video_composer(state: DramaProductionState) -> DramaProductionState:
    ...
    # #10 断点续传：如果视频规划已存在且无需重做，直接跳过
    if _should_skip_node(state, "video_plan"):
        existing = state.get("video_plan") or {}
        await _log(state, "agent", "video_composer", "production",
                   f"视频规划已存在（断点续传跳过，共 {len(existing.get('videos', []))} 个视频提示词）")
        state["feedback_message"] = ""
        state["last_error"] = ""
        return state
...
async def node_lite_storyboard(state: DramaProductionState) -> DramaProductionState:
    ...
    if _should_skip_node(state, "storyboard_data"):
        existing = state.get("storyboard_data") or {}
        await _log(state, "agent", "lite_storyboard", "lite_storyboard",
                   f"轻量分镜已存在（断点续传跳过）")
        state["feedback_message"] = ""
        state["last_error"] = ""
        return state
...
async def _run_reviewer(
    ...
    #7 智能质检重试：如果是重试，注入上次失败原因，让 reviewer 重点关注未修复的问题。
    """
    ...
    if previous_feedback:
        retry_hint = f"\n\n【上次审核未通过原因】请重点检查以下问题是否已修复：\n{previous_feedback}"

    user_content = f"请对 {dimension} 维度进行验收。\n\n待验收数据：\n{json.dumps(context, ensure_ascii=False, indent=2)}{user_feedback_hint}{retry_hint}"
    try:
...
async def _review_node(state: DramaProductionState, stage: str, rules: str, target_agent: str) -> DramaProductionState:
    ...
    if passed:
        ...
        state["retry_count"] = 0
        state["last_error"] = ""
        state["failed_dimensions"] = []  # #7 清空失败维度
        state["previous_review_feedback"] = ""  # #7 清空上次反馈

        # #10 标记阶段验收通过，供断点续传阶段级跳过使用
        completed = state.get("_completed_stages") or []
        if stage and stage not in completed:
            completed.append(stage)
            state["_completed_stages"] = completed
        ...
        state["retry_count"] = (state.get("retry_count") or 0) + 1
        state["last_error"] = ""
        state["failed_dimensions"] = failed_dimensions  # #7 记录失败维度
        state["previous_review_feedback"] = feedback  # #7 记录上次反馈供下次注入
...
def route_after_review_asset(state: DramaProductionState) -> str:
    # #7 智能质检重试：只重跑失败维度对应的 Agent
    if _should_retry(state):
        failed = state.get("failed_dimensions") or []
        # 如果只有 scene 维度失败，只回场景设计师
        if failed == ["scene"]:
            return "scene_prop_designer"
        # 如果只有 character 维度失败，只回角色设计师
        if failed == ["character"]:
            return "character_designer"
        # 两者都失败或未记录维度信息，回到角色设计师重新开始该阶段
        return "character_designer"
...
async def run_stage(
    ...
    # #10 阶段级断点续传：如果该阶段已验收通过且所有产出存在且无用户反馈，跳过整个阶段
    has_user_feedback = bool(state.get("pending_user_feedback"))
    stage_completed = stage in (state.get("_completed_stages") or [])
    if not has_user_feedback and stage_completed and _has_all_stage_outputs(state, stage):
        await _log(state, "system", "director_brain", stage,
                   f"{stage} 阶段已验收通过且产出完整（断点续传跳过整个阶段）")
        final_state = state
    else:
...
async def run_drama_brain(user_instruction: str, options: Optional[Dict[str, str]] = None) -> DramaProductionState:
    ...
    state: DramaProductionState = {
        ...
        "iteration_log": [],
        "_completed_stages": [],       # #10 断点续传
        "_completed_episodes": [],     # #10 集级别断点续传
        "failed_dimensions": [],       # #7 审核失败维度
        "previous_review_feedback": "", # #7 上次审核反馈
    }
