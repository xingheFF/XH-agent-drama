...
def require_canvas_access(
    ...
    if not can_access_canvas(db, str(current_user.id), canvas):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="没有权限访问该画布")
    return canvas


def require_canvas_edit(
    canvas_id: UUID,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    """校验当前用户对指定画布有编辑权限，返回画布对象。"""
    from app.crud import canvas as crud_canvas
    ...
    if not can_edit_canvas(db, str(current_user.id), canvas):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="没有权限编辑该画布")
    return canvas
