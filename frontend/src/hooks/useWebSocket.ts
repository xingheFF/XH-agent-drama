...
export function useWebSocket(options: UseWebSocketOptions = {}): UseWebSocketReturn {
  const { canvasId, enabled = true, onTaskUpdate, onConnect, onDisconnect } = options;
  ...
  const callbacksRef = useRef({ onTaskUpdate, onConnect, onDisconnect });
  /**
  ...
  callbacksRef.current = { onTaskUpdate, onConnect, onDisconnect };
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          if (data.type === 'task_update' && callbacksRef.current.onTaskUpdate) {
            callbacksRef.current.onTaskUpdate(data as TaskUpdate);
          }
        } catch {
          // 忽略非 JSON 消息
        }
